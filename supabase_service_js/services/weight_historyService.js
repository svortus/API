import { insertWeightHistory, updateWeightHistory, selectWeightHistory, selectLastUpdated, selectSinceLastDate } from "../repositories/weight_historyRepository.js";
...
export async function getSinceWeightHistories(req, res) {
    try {
        const date = req.query.date;
        const data = await selectSinceLastDate(date);
        return res.status(200).json({ weightHistories: data });
    } catch (error) {
